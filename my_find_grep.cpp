...
*/
    ...
    int find_or_grep(const char *filename) {
        ...
        if (search_mode == "grep" || search_mode == "grep-binary") {
            // TEST:
            // diff <(awk -F\; '{print $2}' output|sort) <(grep -rI Paraná --color=never -l /nvme/ | sort)
            ...
            int fd;
            if ( exe_name.find("mfgo") != std::string::npos )
                fd = open(filename, O_RDONLY | O_DIRECT); //O_DIRECT: use memalign above
            else
            ...
            if (fd > 0) {
                ...
                //  struct stat st; // for `du` like sum of file sizes
                //  fstat(fd, &st);
                //  sum_file_size += st.st_size;
                while (int n = read(fd, buffer, read_size)) { // read_size == buffer_size - 1; for \0 termination
                    void *position;
                    ...
                    if (position) { // found \0 byte: skip this file
                        hash = -1;
                        //  sum_file_size -= st.st_size; // don't add file we are skipping
                        break;
                    } else {
                        if (search_mode == "grep-binary")
                        ...
                        if (check_saved_buffer) { // for files > buffer_size check end/start of buffers
                            ...
                            if ((searching_for.length() > 1) && (n > 0)) {
                                ...
                                if (pos) {
                                    ...
                                    hash = 272727; // special return value for buffer-corners
                                    
                                }
                            }
                        }

                        if (n == read_size && read_size >= SAVED_BUFFER_HALF_SIZE) { //which means buffer was full and at least buffer-half-bytes
...
#endif
                int hash = find_or_grep(sFileName.c_str()); // FIND or GREP file
